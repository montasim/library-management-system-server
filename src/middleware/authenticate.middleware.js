...
const authenticateMiddleware = (roleCheck = accessTypesConstants.BOTH, requiredPermission = null) => async (req, res, next) => {
    ...
    try {
        ...
        switch (roleCheck) {
            ...
            case accessTypesConstants.BOTH:
                isAuthorized = await validateUserRequest(requester) || await validateAdminRequest(requester);
                break;
            default:
                isAuthorized = false;
        }
        ...
        if (requiredPermission) {
            ...
            if (!hasPermission) {
                ...
                return res.status(httpStatus.UNAUTHORIZED).send(createErrorData('Insufficient permissions.', httpStatus.UNAUTHORIZED, req.originalUrl));
            }
        } else {
        ...
        return res.status(httpStatus.FORBIDDEN).send(createErrorData(`Your session has expired or the token is invalid: ${error.message}. Please login again.`, httpStatus.FORBIDDEN, req.originalUrl));
    }
};
